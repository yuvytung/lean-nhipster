...
describe('Region Controller', () => {
    ...
    it('/POST create regions', async () => {
        const createdEntity: RegionDTO = (
            await request(app.getHttpServer()).post('/api/regions').send(entityMock).expect(201)
        ).body;
    ...
    it('/PUT update regions', async () => {
        const updatedEntity: RegionDTO = (
            await request(app.getHttpServer()).put('/api/regions').send(entityMock).expect(201)
        ).body;
