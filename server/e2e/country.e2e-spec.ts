...
describe('Country Controller', () => {
    ...
    it('/POST create countries', async () => {
        const createdEntity: CountryDTO = (
            await request(app.getHttpServer()).post('/api/countries').send(entityMock).expect(201)
        ).body;
    ...
    it('/PUT update countries', async () => {
        const updatedEntity: CountryDTO = (
            await request(app.getHttpServer()).put('/api/countries').send(entityMock).expect(201)
        ).body;
